...
bool AP_GPS_NMEA::_have_new_message()
{
    ...
    if((now - _last_PAST_ms < 150)&&(_last_PAST_ms != 0)){
        _last_PAST_ms = 1;
        return true;
    }
    if (_last_RMC_ms == 0 ||
...
bool AP_GPS_NMEA::_term_complete()
{
    ...
    if (_term_number == 0) {
        ...
        if (strcmp(term_type, "RMC") == 0) {
            ...
            _gps_data_good = true;
        }else if (strcmp(term_type, "AT") == 0){
            _sentence_type = _GPS_SENTENCE_PSAT;
            _gps_data_good = true;
        } else {
            _sentence_type = _GPS_SENTENCE_OTHER;
        }
    ...
    if (_sentence_type != _GPS_SENTENCE_OTHER && _term[0]) {
        ...
        case _GPS_SENTENCE_RMC + 2: // validity (RMC)
            ...
            break;
        case _GPS_SENTENCE_PSAT + 27: //Fix data (PSAT)
        ...
        // course and speed
        //
        case _GPS_SENTENCE_PSAT + 18:
            _new_speed = _parse_decimal_100(_term);
            break;
